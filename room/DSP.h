...
public:
    ...
    // trims en dB (-6..+6), mode ADVANCED. 0 = comportement auto pur.
    inline float processSample (float x, float amount,
                                float bodyTrimDb=0.0f,
                                float presTrimDb=0.0f,
                                float airTrimDb =0.0f) noexcept
    {
        const bool trims = (std::fabs(bodyTrimDb)+std::fabs(presTrimDb)+std::fabs(airTrimDb))>0.01f;
        if (amount<=0.0001f && !trims) return x;
        ...
        // trims dB → offsets de coefficient (±6 dB ≈ ±facteur doux)
        const float tb=bodyTrimDb/6.0f, tp=presTrimDb/6.0f, ta2=airTrimDb/6.0f;

        // ── CORPS : comble le grave manquant, doux, jamais boueux ─────────
        const float cBody=juce::jmax(-0.30f, amount*(0.15f+0.40f*thin)+tb*0.30f);
        const float body =(std::tanh(low*(1.2f+amount*1.8f))-low)*cBody;

        // ── PRESENCE + AIR : boost lineaire PROPRE, garde anti-sibilance ──
        const float cPres=juce::jmax(-0.22f, amount*0.22f*(0.5f+0.5f*dull)+tp*0.22f);
        const float presence=hp3*cPres;
        const float cAir =juce::jmax(-0.60f, amount*(0.30f+0.80f*dull)+ta2*0.55f);
        const float air  =hp8*cAir*sibGuard;

        // ── FILL : saturation TRES legere, uniquement si signal pauvre ────
...
public:
    ...
    void process (juce::AudioBuffer<float>& buf, float compAmt, float sibAmt,
                  float sibFreqHz=7000.0f) noexcept
    {
        // Retune du de-esser si la frequence a change (mode ADVANCED)
        sibFreqHz=juce::jlimit(5000.0f,9000.0f,sibFreqHz);
        if (std::fabs(sibFreqHz-sibFreqCur)>25.0f)
        {
            sibFreqCur=sibFreqHz;
            for (auto& b:sibHP) b.setHP(sr,(double)(sibFreqCur*0.886f),0.707);
        }
